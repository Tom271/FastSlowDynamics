...
\\
It is readily checked that the coordinate transfomation is correct by evaluating (\ref{FoldConditions}). It is clear to see that $\folds=(0,0)$, and differentiation of $f$ yields $\pd[2]{f}{x}(0,0,0)=2>0$ and $\pd[1]{f}{y}(0,0,0)=-1<0$. Furthermore, $g(0,0,0) = -1 <0$. Therefore, the new system of equations posesses the required qualities.

\subsection{Reduced Dynamics}

In order to determine the reduced dynamics on the critical manifold $S$, Equation \ref{slowsystem} in the limit $\epsilon\to 0$ is considered which yields the following system,
\begin{subequations}
    \begin{align}
