...
The aim is to find a coordinate transformation that satisfies the conditions in (\ref{FoldConditions}). The proposed transformation is $(x,y)\to (1-\tilde{x},\tilde{y}-\frac{2}{3})$, which represents a reflection and a translation of the system \st $(x_0^+,y_0^+)$ is mapped to $(0,0)$. \\



Now using the proposed mapping $(x,y)\to (1-\Tilde{x},\Tilde{y}-\frac{2}{3})$ we are able to redefine the fast system (Equation \ref{fastsystem}) in the following way, 
\begin{equation}
    \begin{cases}
        x'=-y+x^2-\dfrac{(x)^3}{3}\\
        y'=\epsilon(x-1),
...
\end{equation}
where the tilde has been dropped on $x \ \text{and} \ y$ for convenience. The slow system (Equation \ref{slowsystem}) is redefined as
\begin{equation}\label{eq: Slow System}
    ...
    \end{cases}
\end{equation}
using the normal rescaling of time. These two systems will be used throughout the following analysis of the generic fold point. We should note that we have used the general system (Equations \ref{FastS}) to produce the new system where we have chosen $ \lambda=1 $.\\
